# ===  MrT Scalper Combo + Booster  ===

# ===  OKX + Tokocrypto fallback + Debug ===

# ===  VWAP Div + RSI Opt + ADX/BB/Stoch/Candle/Hybrid + Swift Algo ===

# =====================================

import logging
import os
import json
import requests
import pandas as pd
import ta
from datetime import datetime

# -------------------------

# CONFIG

# -------------------------

TIMEFRAMES = ["15m", "1h"]

SYMBOLS = [
"BTCUSDT", "ETHUSDT", "BNBUSDT", "SOLUSDT", "XRPUSDT",
"ADAUSDT", "DOGEUSDT", "AVAXUSDT", "DOTUSDT", "LINKUSDT",
"TRXUSDT", "OKBUSDT", "LTCUSDT", "SHIBUSDT", "UNIUSDT",
"ATOMUSDT", "BCHUSDT", "FILUSDT", "ETCUSDT", "APTUSDT",
"NEARUSDT", "ARBUSDT", "OPUSDT", "ICPUSDT", "TONUSDT",
"AAVEUSDT", "SANDUSDT", "THETAUSDT", "EGLDUSDT", "FLOWUSDT",
"COREUSDT", "XTZUSDT", "MANAUSDT", "GALAUSDT", "PEPEUSDT",
"SNXUSDT", "CRVUSDT", "INJUSDT", "XLMUSDT", "CFXUSDT",
"CHZUSDT", "NEOUSDT", "COMPUSDT", "IMXUSDT", "ZROUSDT",
"WLDUSDT", "SUIUSDT", "PYTHUSDT"
]

TP1_PERCENT = 0.05
TP2_PERCENT = 0.10
SL_PERCENT = 0.05
ENTRY2_ADJUST = 0.005

TELEGRAM_TOKEN = os.environ.get("TELEGRAM_TOKEN")
CHAT_ID = os.environ.get("CHAT_ID")

LAST_SIGNALS_FILE = "last_signals.json"
SIGNALS_CSV = "signals_log.csv"
last_signals = {}

# -------------------------

# LOGGING SETUP

# -------------------------

LOG_LEVEL = os.environ.get("LOG_LEVEL", "INFO").upper()
logging.basicConfig(
level=getattr(logging, LOG_LEVEL, logging.INFO),
format="%(asctime)s %(levelname)s: %(message)s",
handlers=[
logging.FileHandler("bot_debug.log"),
logging.StreamHandler()
]
)
logging.info(f"🔧 Logging level set to {LOG_LEVEL}")

# -------------------------

# TELEGRAM UTIL

# -------------------------

def send_message(msg: str):
"""Send a Markdown-formatted message to Telegram (if configured)."""
if not TELEGRAM_TOKEN or not CHAT_ID:
logging.debug("Telegram not configured; printing message instead.")
print(msg)
return
try:
url = f"[https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage](https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage)"
payload = {"chat_id": CHAT_ID, "text": msg, "parse_mode": "Markdown"}
resp = requests.post(url, json=payload, timeout=10)
if resp.status_code != 200:
logging.warning(f"Telegram API {resp.status_code}: {resp.text}")
except Exception as e:
logging.error(f"[ERROR] Telegram failed: {e}")

# -------------------------

# FILE UTILITIES

# -------------------------

def load_last_signals():
global last_signals
try:
if os.path.exists(LAST_SIGNALS_FILE):
with open(LAST_SIGNALS_FILE, "r") as f:
last_signals.update(json.load(f))
logging.info(f"Loaded last_signals: {len(last_signals)}")
except Exception as e:
logging.error(f"Failed loading last_signals: {e}")

def save_last_signals():
try:
with open(LAST_SIGNALS_FILE, "w") as f:
json.dump(last_signals, f, indent=2)
except Exception as e:
logging.error(f"Failed saving last_signals: {e}")

def log_signal_csv(row: dict):
df = pd.DataFrame([row])
if os.path.exists(SIGNALS_CSV):
df.to_csv(SIGNALS_CSV, mode="a", header=False, index=False)
else:
df.to_csv(SIGNALS_CSV, index=False)

# -------------------------

# KLINES (OKX primary, Tokocrypto fallback)

# -------------------------

def get_klines(symbol: str, interval: str = "15m", limit: int = 300) -> pd.DataFrame:
"""Fetch klines from OKX; fallback to Tokocrypto (Binance-compatible) if OKX fails."""
okx_tf_map = {
"1m": "1m", "3m": "3m", "5m": "5m", "15m": "15m", "30m": "30m",
"1h": "1H", "4h": "4H", "6h": "6H", "12h": "12H",
"1d": "1D", "1w": "1W"
}
okx_interval = okx_tf_map.get(interval, interval)
inst = symbol.replace("USDT", "-USDT") if "-" not in symbol else symbol

```
okx_endpoints = [
    "https://www.okx.com",
    "https://www.okx.cab",
    "https://www.okx.co",
    "https://aws.okx.com",
]

for base in okx_endpoints:
    try:
        url = f"{base}/api/v5/market/candles"
        params = {"instId": inst, "bar": okx_interval, "limit": limit}
        r = requests.get(url, params=params, timeout=10)
        data = r.json()
        if isinstance(data, dict) and "data" in data and len(data["data"]) > 0:
            raw = data["data"][::-1]
            df = pd.DataFrame(raw, columns=[
                "open_time", "open", "high", "low", "close", "volume",
                "volCcy", "volCcyQuote", "confirm"
            ])
            for c in ["open", "high", "low", "close", "volume"]:
                df[c] = df[c].astype(float)
            df["open_time"] = pd.to_datetime(pd.to_numeric(df["open_time"], errors="coerce"), unit="ms")
            df["close_time"] = df["open_time"]
            return df
    except Exception as e:
        logging.debug(f"OKX endpoint {base} failed for {symbol}: {e}")

# fallback to Tokocrypto
logging.warning(f"⚠️ All OKX endpoints failed for {symbol} — trying Tokocrypto fallback")
try:
    url = "https://api.tokocrypto.com/api/v3/klines"
    params = {"symbol": symbol, "interval": interval, "limit": limit}
    r = requests.get(url, params=params, timeout=10)
    data = r.json()
    if isinstance(data, list) and len(data) > 0:
        raw = data[::-1]
        df = pd.DataFrame(raw, columns=[
            "open_time", "open", "high", "low", "close", "volume",
            "close_time", "quote_asset_volume", "trades", "taker_base", "taker_quote", "ignore"
        ])
        for c in ["open", "high", "low", "close", "volume"]:
            df[c] = df[c].astype(float)
        df["open_time"] = pd.to_datetime(df["open_time"], unit="ms")
        df["close_time"] = pd.to_datetime(df["close_time"], unit="ms")
        logging.info(f"{symbol}: ✅ Tokocrypto fallback used")
        return df
    else:
        logging.error(f"Tokocrypto returned unexpected data for {symbol}: {data}")
except Exception as e:
    logging.error(f"Tokocrypto fallback failed for {symbol}: {e}")

return pd.DataFrame()
```

# -------------------------

# INDICATORS & SIGNAL DETECTORS

# -------------------------

def detect_signal(df: pd.DataFrame, interval: str = "1h"):
if df.shape[0] < 50:
return None
try:
df["ema50"] = ta.trend.EMAIndicator(df["close"], window=50).ema_indicator()
df["ema200"] = ta.trend.EMAIndicator(df["close"], window=200).ema_indicator()
df["rsi"] = ta.momentum.RSIIndicator(df["close"], window=14).rsi()
macd = ta.trend.MACD(df["close"])
df["macd"] = macd.macd()
df["macd_signal"] = macd.macd_signal()
except Exception as e:
logging.error(f"Indicator calc error: {e}")
return None

```
last = df.iloc[-1]
if last["ema50"] > last["ema200"] and last["macd"] > last["macd_signal"] and last["rsi"] > 55:
    return "BUY", "Trend Confirm", "Strong"
elif last["ema50"] < last["ema200"] and last["macd"] < last["macd_signal"] and last["rsi"] < 45:
    return "SELL", "Trend Confirm", "Strong"
return None
```

def detect_signal_15m(df: pd.DataFrame):
if df.shape[0] < 60:
return None
try:
df["rsi"] = ta.momentum.RSIIndicator(df["close"], window=14).rsi()
macd = ta.trend.MACD(df["close"])
df["macd"] = macd.macd()
df["macd_signal"] = macd.macd_signal()
df["vwap"] = ta.volume.VolumeWeightedAveragePrice(high=df["high"], low=df["low"], close=df["close"], volume=df["volume"]).volume_weighted_average_price()
df["obv"] = ta.volume.OnBalanceVolumeIndicator(df["close"], df["volume"]).on_balance_volume()
df["obv_ma"] = df["obv"].rolling(20).mean()
df["volume_ratio"] = df["volume"] / df["volume"].rolling(20).mean()
except Exception as e:
logging.error(f"Indicator error 15m: {e}")
return None

```
last = df.iloc[-1]
base_buy = (
    last["rsi"] > 50 and last["macd"] > last["macd_signal"] and
    last["close"] > last["vwap"] and last["obv"] > last["obv_ma"] and
    last["volume_ratio"] > 1.0
)
base_sell = (
    last["rsi"] < 50 and last["macd"] < last["macd_signal"] and
    last["close"] < last["vwap"] and last["obv"] < last["obv_ma"] and
    last["volume_ratio"] > 1.0
)

if base_buy:
    return "BUY", "RSI+MACD+VWAP+OBV", "Strong", {"score": 6, "reasons": ["RSI>50", "MACD>Signal"]}
elif base_sell:
    return "SELL", "RSI+MACD+VWAP+OBV", "Strong", {"score": 6, "reasons": ["RSI<50", "MACD<Signal"]}
return None
```

# -------------------------

# Swift Algo Booster

# -------------------------

def detect_signal_swift(df: pd.DataFrame):
if df.shape[0] < 30:
return None
try:
df["ema9"] = ta.trend.EMAIndicator(df["close"], window=9).ema_indicator()
df["ema21"] = ta.trend.EMAIndicator(df["close"], window=21).ema_indicator()
df["rsi_fast"] = ta.momentum.RSIIndicator(df["close"], window=7).rsi()
df["high_look"] = df["high"].rolling(20).max()
df["low_look"] = df["low"].rolling(20).min()
except Exception as e:
logging.error(f"Swift algo calc error: {e}")
return None

```
last = df.iloc[-1]
if last["ema9"] > last["ema21"] and last["rsi_fast"] > 55 and last["close"] > last["high_look"] * 0.995:
    return "BUY"
elif last["ema9"] < last["ema21"] and last["rsi_fast"] < 45 and last["close"] < last["low_look"] * 1.005:
    return "SELL"
return None
```

def confirm_signal(signal_small_tf, signal_big_tf):
if not signal_small_tf or not signal_big_tf:
return None
s_signal, s_mode, s_strength, s_meta = signal_small_tf
b_signal, b_mode, b_strength = signal_big_tf
if s_signal == b_signal:
return s_signal, f"Strong Confirmed", f"{s_mode}+{b_mode}", s_meta
return None

# -------------------------

# SCANNER

# -------------------------

def scan_once():
summary = {"buy": 0, "sell": 0}
for symbol in SYMBOLS:
try:
df15 = get_klines(symbol, "15m")
df1h = get_klines(symbol, "1h")
if df15.empty or df1h.empty:
logging.debug(f"{symbol}: data empty")
continue

```
        res15 = detect_signal_15m(df15)
        res1h = detect_signal(df1h, "1h")
        if not res15 or not res1h:
            continue

        result = confirm_signal(res15, res1h)
        if not result:
            continue

        signal, strength_label, meta = result
        score = meta.get("score", 0.0)

        # Swift booster (doesn't replace core rules)
        swift = detect_signal_swift(df15)
        if swift == signal:
            score += 2
            strength_label += " + Swift Confirmed 🚀"
        elif swift and swift != signal:
            strength_label += " ⚠️ Swift Divergence"

        last_row = df15.iloc[-1]
        entry1 = float(last_row["close"])
        entry2 = entry1 * (1 - ENTRY2_ADJUST) if signal == "BUY" else entry1 * (1 + ENTRY2_ADJUST)
        tp1 = entry1 * (1 + TP1_PERCENT) if signal == "BUY" else entry1 * (1 - TP1_PERCENT)
        tp2 = entry1 * (1 + TP2_PERCENT) if signal == "BUY" else entry1 * (1 - TP2_PERCENT)
        sl = entry1 * (1 - SL_PERCENT) if signal == "BUY" else entry1 * (1 + SL_PERCENT)

        # skip repeat signals
        if last_signals.get(symbol) == signal:
            logging.debug(f"{symbol}: same as last signal, skipping")
            continue

        # Build telegram message with Markdown formatting and line breaks
        emoji = "🟢" if signal == "BUY" else "🔴"
        msg = (
            f"{emoji} *{signal} Signal*\n"
            f"Strength: *{strength_label}*\n"
            f"Score: `{score:.2f}`\n"
            f"Pair: `{symbol}` | TF: `15m & 1h`\n"
            f"Entry 1: `{entry1:.6f}` | Entry 2: `{entry2:.6f}`\n"
            f"TP1: `{tp1:.6f}` | TP2: `{tp2:.6f}`\n"
            f"SL: `{sl:.6f}`\n"
            f"Reasons: {', '.join(meta.get('reasons', []))}\n"
            f"Time: {last_row['close_time'].strftime('%Y-%m-%d %H:%M:%S UTC')}\n"
            f"_Info only — no auto order._"
        )
        send_message(msg)

        # record and log
        last_signals[symbol] = signal
        summary["buy" if signal == "BUY" else "sell"] += 1
        log_signal_csv({
            "time_utc": last_row["close_time"].strftime('%Y-%m-%d %H:%M:%S'),
            "pair": symbol,
            "signal": signal,
            "score": score,
            "entry1": entry1,
            "entry2": entry2,
            "tp1": tp1,
            "tp2": tp2,
            "sl": sl
        })

    except Exception as e:
        logging.error(f"{symbol}: error scanning: {e}")

# summary message
total = summary["buy"] + summary["sell"]
summary_msg = (
    "✅ Scan selesai.\n\n"
    "📊 *Summary Sinyal Baru:*\n"
    f"🟢 Naik (BUY): {summary['buy']}\n"
    f"🔴 Turun (SELL): {summary['sell']}\n"
    f"📈 Total: {total}"
)
send_message(summary_msg)
logging.info(f"Summary: BUY={summary['buy']} SELL={summary['sell']} TOTAL={total}")
return summary
```

# -------------------------

# MAIN

# -------------------------

def main():
load_last_signals()
start_msg = f"🚀 MrT Combo+Booster aktif — {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S UTC')}"
send_message(start_msg)
scan_once()
save_last_signals()

if **name** == "**main**":
main()
 
